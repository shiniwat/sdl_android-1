...
public abstract class SdlBroadcastReceiver extends BroadcastReceiver{
	...
	public void onReceive(Context context, Intent intent) {
		...
		//if (action.equalsIgnoreCase("com.smartdevicelink.broadcast")) {
		//	String config = intent.getStringExtra("TRANSPORT_CONFIG");
		//	Log.d(TAG, "Got braodcast: transport=" + config);
		//}
        if(!(action.equalsIgnoreCase(BOOT_COMPLETE)
				...
				|| action.equalsIgnoreCase(SdlRouterService.REGISTER_NEWER_SERVER_INSTANCE_ACTION))){
        	//We don't want anything else here if the child class called super and has different intent filters
        	DebugTool.logInfo("Unwanted intent from child class");
        	return;
        }
        
        if(action.equalsIgnoreCase(USBTransport.ACTION_USB_ACCESSORY_ATTACHED)){
        	DebugTool.logInfo("onReceive: Usb connected");
			// why action is NULL??
			// @REVIEW let's keep action ACTION_USB_ACCESSORY_ATTACHED
		...
		if(action.equalsIgnoreCase(TransportConstants.START_ROUTER_SERVICE_ACTION) || action.equalsIgnoreCase(TransportConstants.START_AOA_ROUTER_SERVICE_ACTION)){
			...
			if(intent.hasExtra(TransportConstants.START_ROUTER_SERVICE_SDL_ENABLED_EXTRA)){
				...
				if(intent.getBooleanExtra(TransportConstants.START_ROUTER_SERVICE_SDL_ENABLED_EXTRA, false)){
                    Log.v(TAG, "     with SDL_ENABLED == true");
                    ...
                    Log.v(TAG, "     with SDL_ENABLED == false");
				}
				return;
			}else if(intent.getBooleanExtra(TransportConstants.PING_ROUTER_SERVICE_EXTRA, false)){
				Log.v(TAG, " in package " + context.getPackageName() + " received 'ping' broadcast '" + action + "'");
		...
		if(Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
			...
			ServiceFinder finder = new ServiceFinder(context, context.getPackageName(), new ServiceFinder.ServiceFinderCallback() {
				...
				public void onComplete(Vector<ComponentName> routerServices) {
					...
					// we need to exclude AoaRouterService
					for (ComponentName router: routerServices) {
						//if (router.toString().toLowerCase().contains(SDL_AOA_ROUTER_SERVICE_CLASS_NAME)) {
						//	Log.d(TAG, "exclude AOA router: " + router);
						//} else {
							runningRouterServicePackage.add(router);
						//}
					}
	...
	private static boolean isRouterServiceRunning(Context context, boolean pingService, TransportType transportType){
		...
		for (RunningServiceInfo service : runningServices) {
	    	...
	    	if ((service.service.getClassName()).toLowerCase(Locale.US).contains(SDL_ROUTER_SERVICE_CLASS_NAME) && AndroidTools.isServiceExported(context, service.service) && (transportType == TransportType.MULTIPLEX || transportType == TransportType.MULTIPLEX_AOA)) {
		...
		if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
			ServiceFinder finder = new ServiceFinder(context, context.getPackageName(), new ServiceFinder.ServiceFinderCallback() {
				...
				public void onComplete(Vector<ComponentName> routerServices) {
					...
					requestTransportStatus(context,null,false, transportType);
				}
			});
	...
	private static void requestTransportStatus(Context context, final SdlRouterStatusProvider.ConnectedStatusCallback callback, final boolean triggerRouterServicePing, final TransportType transportType){
		...
		if(transportType.equals(TransportType.MULTIPLEX) && isRouterServiceRunning(context,false,transportType) && !runningRouterServicePackage.isEmpty()){	//So there is a service up, let's see if it's connected
			DebugTool.logInfo("router service is running...");
